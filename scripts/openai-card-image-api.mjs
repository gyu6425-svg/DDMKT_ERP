...
function getReferenceImages(payload) {
    return Array.isArray(payload.imageDataUrls)
        ? payload.imageDataUrls.filter((imageDataUrl) => Boolean(imageDataUrl))
        : [payload.imageDataUrl].filter((imageDataUrl) => Boolean(imageDataUrl));
}

function getSeriesStyleReferenceImages(payload) {
    return Array.isArray(payload.seriesStyleReferenceImageDataUrls)
        ? payload.seriesStyleReferenceImageDataUrls.filter((imageDataUrl) => Boolean(imageDataUrl))
        : [];
}
