# 다른 PC에서 카페 원고 생성 · 카페 발행 돌리기

회사의 다른 컴퓨터에 **카페 담당** 역할을 설치하는 안내서.
**위에서부터 순서대로** 따라 하면 됩니다.

> **이 PC가 맡는 일 = 카페 원고 생성 + 카페 발행, 이 둘뿐입니다.**
> 크롤링(순위 측정)은 **메인 PC가 계속 담당**하므로 여기서 하지 않습니다. (이유는 0번 참고)
...
| 역할 | 담당 | 비고 |
|---|---|---|
| 블로그 순위 크롤 (새벽 3시) | **메인 PC** | 제일 무거움. 5시간 30분 연속 |
| 당일글 크롤 (30분마다) | **메인 PC** | |
| 플레이스·카페 순위 크롤 | **메인 PC** | |
| 재검색 리스너 | **메인 PC** | |
| **카페 원고 생성** | **이 PC** ✅ | 이 문서로 설치 |
| **카페 발행** | **이 PC** ✅ | 이 문서로 설치 (9단계) |

### 왜 크롤은 여기서 안 하나

**나눠 봐야 이득이 없고, 오히려 위험합니다.**

1. **같은 사무실 = 같은 공인 IP.** 네이버 차단은 IP 기준이라 두 PC로 나눠도 분산이 안 됩니다.
   크롤러엔 일부러 속도 조절(요청당 3.5초, 6개마다 40초 휴식)을 넣어뒀는데,
   두 대가 동시에 돌면 **같은 IP에서 2배 속도로 긁는 꼴**이 되어 차단 위험만 커집니다.
2. **크롤 시간표가 겹치지 않게 정교하게 짜여 있습니다.**
   `03:00 전체크롤 → 08:30 종료` / `09:05 당일글` / `09:20 플레이스`.
   과거 09:00~09:20 구간이 겹쳐서 전체크롤이 계속 죽었던 이력이 있어(0xC000013A)
   마감을 08:30으로 당겨 30분 여유를 만든 것입니다.
   두 PC로 나누면 서로 뭘 하는지 몰라서 **이 겹침을 막을 방법이 없습니다.**
3. **같은 Supabase에 기록**하므로 양쪽에서 돌리면 중복 측정이 됩니다.

반면 **카페 작업은 성격이 완전히 달라** 나누기 좋습니다:
- 크롤 = 네이버 검색을 **대량으로 긁음** (차단 민감)
- 카페 = 로그인해서 **글 하나 씀** (검색 거의 안 씀)

### ⚠️ 절대 규칙

- **카페 발행 리스너는 한 대에서만.** 두 대에서 켜면 **같은 글이 두 번 올라갑니다.**
  → 그래서 9-1 에서 메인 PC의 발행을 먼저 내립니다.
- **크롤은 여기서 돌리지 마세요.** (메인 PC 담당)
- 카페 원고 생성은 네이버 검색을 조금 씁니다(지역당 1~2회). 되도록
  **새벽 3시~8시 30분(메인 PC 전체크롤 시간대)은 피해서** 낮·저녁에 돌리세요.
...
블로그 순위·당일글·플레이스·카페 순위 크롤은 **전부 메인 PC 담당**입니다.
(이유는 0번 "왜 크롤은 여기서 안 하나" 참고 — 같은 IP라 나눠도 이득이 없고,
크롤끼리 겹치면 죽는 문제 때문에 시간표가 정교하게 짜여 있습니다.)

이 PC에서는 **`crawler` 폴더의 크롤 스크립트를 실행하지 마세요.**
(`blog_rank_crawler.py`, `crawl_bydate.py`, `place_rank_crawler.py`, `cafe_rank_crawler.py`)
...
- **`.bat` 파일 대부분은 이 PC에서 더블클릭해도 안 됩니다.**
  파일 안에 **메인 PC의 파이썬 설치 경로**(`C:\Users\ddmkt\...`)가 그대로 적혀 있어서,
  사용자명이 다른 PC에서는 실행이 안 됩니다.
  → **원고 생성은 `python cafe_auto_publish.py` 처럼 직접 실행**하세요.
  → 단, 9단계의 `run_chrome_login.bat` / `run_cafe_listener.bat` 은 발행에 필요하므로,
    실행이 안 되면 아래처럼 직접 실행하면 됩니다:
    ```
    cd DDMKT_ERP\crawler\cafe_pub
    python publish_listener.py
    ```
    (크롬은 `run_chrome.bat` 내용 중 chrome.exe 줄을 직접 실행하거나,
     크롬 설치 경로를 그 PC에 맞게 고쳐서 쓰세요.)
- **크롤 스케줄은 이 PC에 만들지 마세요.** 메인 PC 담당입니다.
