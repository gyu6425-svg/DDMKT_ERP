...
export function buildReviewPrompt(payload: GenerateCafePayload): string {
    ...
    return [
        ...
        `- 소제목(부제목): 「사진 N」 마커 바로 다음 줄에, 이어질 문단에 어울리는 소제목을 "부제목 : <내용>" 형식(한 줄 단독)으로 넣어라. 전부는 아니고 사진들 중 절반가량에만(질문형 "~일까요?" 과 핵심 혜택형을 섞어서). 그중 1~2개는 지역명(${region})+키워드를 자연스럽게 포함해 검색 노출에 도움되게. 첫 사진(사진 1)은 소제목 없이 인사말로 시작, 마지막 상담 유도 문장 바로 앞에도 마무리형 "부제목 : ..." 한 줄.`,
