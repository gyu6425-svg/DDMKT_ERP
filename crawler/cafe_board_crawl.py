...
#   ★ 마이클의 정보세상(ddmkt2, club 31754130)은 2026-08-18 사장님 지시로 '설고점만' 남긴다.
#     누수·더맨시스템·더티클리닉 게시판은 추적 중단(그 카페엔 더 발행하지 않는다·마지막 발행 08-04).
#     옛 글은 excluded=true 로 내렸다. 더맨은 자체카페(themansys)로 계속 추적된다.
#     ※ ddmkt2 는 순위 측정 자체가 2026-08-06 부터 제외돼 있다(cafe_rank_crawler) — 설고점 ddmkt2 글도
#       트래커에 보이기만 하고 매일 재측정은 안 된다. 크롤 부하와 무관.
TARGETS = [
    ("31754130", "ddmkt2", "2", "설고점", "seolgo"),          # 마이클 정보세상 - 설고점만 유지
