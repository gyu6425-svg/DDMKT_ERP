...
#    ⚠ '서울 소방업체' 제외(2026-08-19) — 네이버가 이 검색어에 광고를 넣어 인기글 섹션 자체가 사라졌다.
#      우리 코드 회귀가 아니라 네이버 지면 변경이다. 그대로 두니 5일 연속(8/14~8/18) '골든 11/12' alert 가
#      떴고, 매일 뜨는 경보는 아무도 안 본다 — 진짜 회귀가 났을 때 묻힌다. 그래서 뺀다.
#      나머지 소방업체 3건은 계속 통과하므로 회귀 감지용으로 남긴다(스캔 계획 #4 는 중지했지만
#      카나리는 '판정 로직이 살아있나'를 보는 것이라 상용 스캔 여부와 무관하다).
GOLDEN = [
    "강남 소방업체", "광진 소방업체", "금천 소방업체",
    "강남 누수탐지", "수원 누수탐지",
    "강남 입주청소", "송파 입주청소",
    "인천 줄눈시공", "강남 줄눈시공",
    "서울 간병인", "수원 간병인",
