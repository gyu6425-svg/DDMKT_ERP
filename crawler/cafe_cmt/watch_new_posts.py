...
# 제목 필터용 별칭 — 발행 제목 형식이 바뀌어도 그 업종 글을 계속 잡기 위함.
#   누수 글 제목이 '광진 누수탐지 …' 에서 '광진 천장 누수 …' 로 바뀌면서(키워드 '누수탐지'가
#   제목에 없어) 통째로 건너뛰던 문제. 댓글 '문구'에는 여전히 감시행 keyword(누수탐지)를 쓴다
#   — 필터 매칭과 문구 키워드는 별개다.
KEYWORD_ALIASES = {
    "누수탐지": ("누수탐지", "천장 누수", "누수"),
    "보안": ("보안",),
...
def _title_matches(keyword, title):
    """제목이 이 업종 글인가 — 키워드 또는 그 별칭 중 하나라도 제목에 있으면 True."""
