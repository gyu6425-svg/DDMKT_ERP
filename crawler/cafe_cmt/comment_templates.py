...
TEMPLATES_CATERING = [
    '{지역} 출장뷔페 후기 잘 봤습니다. 음식이 정갈하네요!',
    '안그래도 {지역} 출장뷔페 알아보고 있었는데 도움 많이 됐어요.',
    '이런 정보 찾고 있었는데 잘 보고 갑니다. {지역} 출장뷔페 참고할게요!',
    '집에서 모임 준비하려니 막막했는데 {지역} 출장뷔페 정보 감사합니다!',
    '{지역} 출장뷔페 메뉴 구성이 알차네요. 다음 행사 때 써봐야겠어요.',
    '정리 깔끔하게 잘 해주셨네요. {지역} 출장뷔페 필요하면 참고하겠습니다.',
    '돌잔치 준비 중인데 {지역} 출장뷔페 후기라 반갑네요. 잘 읽었습니다!',
    '필요한 정보였는데 {지역} 출장뷔페에 대해 깔끔하게 정리해주셔서 감사합니다!',
    # 질문형
    '{지역} 출장뷔페 혹시 최소 몇 인분부터 가능한가요?',
    '{지역}쪽도 출장 되나요? 집들이 때문에 출장뷔페 알아보는 중이에요',
    '{지역} 출장뷔페 예약은 며칠 전에 미리 해야 하나요?',
]
...
# 보안 업종의 '세부 서비스어' — 제목에 있는 걸 그대로 댓글에 쓴다(행사보안/사설경호/건물보안…).
#   더맨시스템은 사설경호·행사보안·건물보안을 다 다뤄서, 제목에 맞는 말을 써야 자연스럽다.
#   순서=우선순위(더 구체적인 말 먼저). 아무 것도 없으면 '보안'.
SECURITY_TERMS = ['행사보안', '사설경호', '개인경호', '신변보호', '건물보안',
                  '시설경비', '시설보안', '경호', '경비', '보안']


def security_keyword(title, default='보안'):
    """보안 글 제목에서 세부 서비스어를 뽑는다. 없으면 default('보안')."""
    t = (title or '')
