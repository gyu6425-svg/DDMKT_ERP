...
_LAST_YIELD_CHECK = 0.0


def _yield_to_immediate_search():
    """크롤 중 사용자 즉시검색(measure_requests)이 대기/처리 중이면 크롤을 잠깐 멈춰 양보한다.
    크롤과 즉시검색이 네이버에 동시 요청하면 소프트 차단으로 즉시검색이 빈응답(fail) 나기 때문 —
    라이브 검색이 배경 크롤보다 우선. DB 조회는 4초에 한 번만(오버헤드 방지)."""
    global _LAST_YIELD_CHECK
    now = time.time()
    if now - _LAST_YIELD_CHECK < 4:
        return
    _LAST_YIELD_CHECK = now
    waited = 0
    while waited < 45:
        try:
            pend = sb_get("measure_requests", {"status": "in.(pending,processing)", "select": "id", "limit": "1"})
        except Exception:
            return
        if not pend:
            break
        if waited == 0:
            print("  ⏸ 즉시검색 대기 감지 — 크롤 잠시 양보(겹침 차단 방지)", flush=True)
        time.sleep(4)
        waited += 4
    _LAST_YIELD_CHECK = time.time()


def _pause(base=None):
    """요청 간격 + 무작위 지터. 일정한 주기는 봇으로 탐지되기 쉬우므로 매번 살짝 흔든다(차단 예방).
    또한 사용자 즉시검색이 대기 중이면 여기서 크롤을 양보한다(_pause 는 모든 크롤 요청 사이의 공통 지점)."""
    _yield_to_immediate_search()
