...
REM      SelfhostBackup added 2026-08-19 - it is now the ONLY backup of the live DB; if it silently
REM      stays Disabled after a reboot, nobody notices until a restore is needed.
...
powershell -NoProfile -Command "foreach($n in 'DDMKT-Crawl-Full','DDMKT-Crawl-Place','DDMKT-Crawl-Today-WD','DDMKT-Crawl-Today-WE','DDMKT-SelfhostBackup'){ try{ Enable-ScheduledTask -TaskName $n -ErrorAction Stop | Out-Null; Write-Output ('enabled ' + $n) } catch { Write-Output ('FAILED ' + $n) } }" >> postboot.log 2>&1
