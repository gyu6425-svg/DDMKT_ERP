...
def region_tokens(road, jibun):
    ...
    for m in re.finditer(r"([가-힣]{1,3}구)(?=\s)", text):  # 상록구
        push(m.group(1))
    # 지번의 법정동: 이동. ★ '성수동2가'·'종로1가'처럼 뒤에 街가 붙는 형태가 서울에 아주 많은데,
    #   동 뒤에 공백만 허용하던 옛 규칙은 이걸 통째로 놓쳤다(실측 2026-08-06: 성수동2가 → 성수동 누락.
    #   그 업체의 핵심 토큰이라 '성동구 약국'만 남고 '성수동 약국'을 못 봤다).
    #   기본형(성수)도 함께 넣는다 — 상권명은 '동'을 떼고 검색하는 경우가 많다(성수 약국).
    for m in re.finditer(r"(?:^|(?<=\s))([가-힣]{1,3})동(?:\d+가)?(?=\s|$)", text):
        push(m.group(1) + "동")
        if len(m.group(1)) >= 2:
            push(m.group(1))
    # 로(路)+街 지명 — '을지로3가'·'종로1가'. 동이 아니라 도로명이라 위 규칙에 안 걸리는데,
    #   실제로는 그 자체가 동네 이름으로 검색된다(을지로3가 맛집). 안 넣으면 중구 하나만 남는다.
    for m in re.finditer(r"(?:^|(?<=\s))([가-힣]{1,3}로)\d+가(?=\s|$)", text):
        push(m.group(0).strip())
        push(m.group(1))
    # 읍·면 — 옥도면 같은 농어촌 소재지. 이게 없어서 '전북 군산시 옥도면 …' 에서 옥도면이 통째로 빠졌다.
    #   접미 포함형(옥도면)과 기본형(옥도) 둘 다 — 실제 검색은 둘 다 쓰인다.
    for m in re.finditer(r"(?:^|(?<=\s))([가-힣]{1,4})(?:읍|면)(?=\s)", text):
        push(m.group(0).strip())
        push(m.group(1))
    # 리(里) → 기본형. 섬·상권명이 여기 들어있는 경우가 많다(선유도리 → 선유도).
    for m in re.finditer(r"(?:^|(?<=\s))([가-힣]{2,4})리(?=\s)", text):
        push(m.group(1))
    # ★ 도로명은 반드시 어절 첫 글자부터 잡는다 — 경계가 없으면 이름 중간부터 매칭된다.
    #   실측(2026-08-06): '마린시티1로' 에서 앞 글자가 잘려 '린시티'가 나왔다(4글자 도로명은 통째로 누락).
    #   길이도 {2,3}→{2,5} 로 넓힌다(마린시티·상록수북 같은 4~5자 도로명).
    for m in re.finditer(r"(?:^|(?<=\s))([가-힣]{2,5})\d*(?:로|길)(?=\s)", text):  # 광덕1로→광덕
        t = m.group(1)
