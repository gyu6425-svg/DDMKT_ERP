...
_SECTION_POOL = [
    "겪을 수 있는 증상·의심 신호와 우선 확인 순서",
    "원인 구분(수도관·배수관·방수층·결로 등)",
    "자가 점검으로 알 수 있는 것과 그 한계",
    "전문 탐지가 필요한 시점과 판단 기준",
    "탐지·수리 진행 절차",
    "이웃·아랫집·관리사무소와의 연락과 책임 구분",
    "비용에 영향을 주는 요소",
    "예방과 평소 관리·유지보수 수칙",
    "계절·기후(장마·겨울 결로 등)에 따른 차이",
    "탐지 장비·방법의 종류와 원리",
    "흔한 오해와 잘못된 대처",
    "누수 발견 직후 초기 대처",
    "아파트·빌라·상가 등 건물 유형별 차이",
    "보험·하자보수, 전세/자가 상황별 참고",
]
# 도입 스타일도 매 글 랜덤 — 같은 리프레임 문구 반복 방지. {region}/{business} 치환.
_INTRO_STYLES = [
    '첫 문단 = "{region} {business}" 대표키워드로 시작하는 명제 + 구체적 상황 2~3가지. 둘째 문단 = 눈에 보이는 것만으로 판단하기 어려운 이유를 짚고 글의 방향을 자연스럽게 제시.',
    '첫 문단 = "{region} {business}"로 시작해 이 지역·건물에서 흔히 마주치는 장면을 짧게 언급(1인칭 아님). 둘째 문단 = 왜 원인 파악이 먼저인지 + 무엇을 다룰지.',
    '첫 문단 = "{region} {business}"로 시작한 뒤 천장 얼룩·물소리 같은 신호를 어떻게 봐야 할지 던진다. 둘째 문단 = 성급한 판단의 위험 + 글의 흐름 안내.',
    '첫 문단 = "{region} {business}"로 시작하되 상황 나열보다 "무엇을 알면 손해·재시공을 줄이는지" 각도로. 둘째 문단 = 이 글에서 확인할 포인트.',
    '첫 문단 = "{region} {business}"로 시작하되 비슷한 증상도 원인이 다를 수 있다는 점을 환기. 둘째 문단 = 그래서 무엇부터 따져봐야 하는지 예고.',
]
# 제목 뒷부분에 넣을 '장면/상황' 키워드 — 매 글 랜덤. "{지역} 누수탐지" 뒤에 붙어 제목·검색어를 다양화한다.
_TITLE_SCENES = [
    "아파트 누수현장", "화장실 누수", "천장 물샘", "벽면 누수", "베란다 누수",
    "주방 싱크대 누수", "배관 누수", "옥상 방수", "아랫집 누수", "빌라 누수",
    "결로 습기 문제", "누수 원인 확인", "정밀 탐지현장",
]
...
def _info_prompt(region, count, ending="총정리"):
    ...
    return "\n".join([
        ...
        f'좋은 예: "{region} {BUSINESS} {scene} 원인 구분부터 처리까지 총정리" / "{region} {BUSINESS} {scene} 의심 증상 체크리스트". '
...
def _tags(region, limit=6):
    ...
    rj = region.replace(" ", "")
    tags = [f"{rj}누수탐지", f"{rj}누수", f"{rj}누수탐지업체", f"{rj}화장실누수",
            "누수탐지", "누수", "누수탐지업체", "화장실누수", "아파트누수", "든든한누수탐지"]
...
def load_fixed():
    ...
    CAFE_FIXED_DIR 로 폴더 지정(기본 cafe-fixed). CAFE_SPLIT_COMPOSITE=1 이면 좌우 합성 프레임을 개별 사진으로 분할."""
...
def make_and_queue(region, pool, popular_verified=False):
    ...
    repeat_banner = os.environ.get("CAFE_REPEAT_BANNER", "0").lower() in ("1", "true", "yes")
    count = len(fixed) + (2 if repeat_banner else 1)
    ...
    include_link = random.random() < link_rate
    tag_count = int(os.environ.get("CAFE_TAG_COUNT", str(random.randint(4, 7))))
    variant = f"real{len(fixed)}-banner{'2' if repeat_banner else '1'}-link{'1' if include_link else '0'}-tags{tag_count}"
    ...
    # 본문 뒤 링크는 일부 글에만 넣고, 태그도 핵심 4~7개로 축소해 고정 광고 패턴을 완화한다.
    if include_link:
...
def main():
    ...
    _log(f"이미지 풀 {len(pool)}장 (dir={os.environ.get('CAFE_FIXED_DIR') or 'cafe-fixed'}) → 글마다 랜덤 {os.environ.get('CAFE_PHOTOS_PER_POST','6')}장 + 배너 {banner_count}장")
