...
def cols(cards):
    """나란히 놓기. column 은 2개 이상이어야 한다.
       ...
       ★ width_ratio 를 직접 준다. 안 주면 노션이 내용 길이에 맞춰 칸을 다르게 잡아
         카드 폭이 제각각이 된다(실측 2026-08-21). 높이는 글자 수로 맞춘다 —
         카드 높이를 지정하는 방법은 없으므로 한 줄에 들어가는 짧은 설명으로 통일한다."""
    r = round(1.0 / len(cards), 4)
...
def charge_page():
    return [
        ...
        h2("🔷 세 단계로 끝납니다"),
        cols([
            card("📨", "1. 신청", "방식과 건수를 적습니다"),
            card("💬", "2. 금액 통보", "담당자가 알려 드립니다"),
            card("🏦", "3. 입금 · 지급", "입금하면 건수가 들어옵니다"),
        ]),
        div(),

        h2("① 신청하기"),
        cols([
            card("💳", "결제 방식", "계좌이체 · 카드 · 기타"),
            card("🔢", "건수", "필요한 발행 건수"),
        ...
        h2("③ 입금하고 알리기"),
        cols([
            card("🏦", "입금 계좌 확인", "은행 · 계좌번호 · 예금주"),
            card("✅", "계좌이체 완료", "입금자명을 적고 누르기"),
        ]),
...
def deploy_page():
    return [
        ...
        h2("🎒 무엇을 준비해 오셔야 하나"),
        cols([
            card("🏷", "업체명", "글에 그대로 나갑니다"),
            card("🔑", "네이버 계정", "대신 발행하는 데 필요"),
            card("📋", "카페 · 게시판", "어디에 올릴지"),
        ]),
        cols([
            card("🖼", "사진", "배너 · 실사 · 끝 배너"),
            card("🔍", "키워드 또는 지역", "고르시는 방식에 따라"),
            card("🔐", "2단계 인증 여부", "쓰신다면 꼭 알려 주세요"),
        ]),
        ...
        h2("📦 배포 종류"),
        cols([
            card("📄", "일반 배포", "적어 주신 키워드 그대로 발행합니다", "gray_background"),
            card("🔥", "인기탭 배포", "인기글에 들어갈 키워드만 골라 발행합니다", "green_background"),
        ]),
        ...
        cols([
            card("📍", "지역형", "지역 + 제품 키워드"),
            card("🏪", "키워드형", "네이버 플레이스 주소"),
            card("✍️", "직접 입력형", "키워드 목록"),
            card("🌐", "정보형", "대표 단어 또는 주소"),
        ]),
        ...
        para(t("실사 사진은 많을수록 좋습니다. 올리면 자동으로 압축됩니다. 사진이 없어도 접수는 되지만 글 품질이 떨어집니다.",
               color="gray")),
        div(),
