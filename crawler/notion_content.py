...
def deploy_page(ids=None):
    return [
        ...
        img("08-발행정보.png", "카페 발행 정보 — 네이버 계정과 게시판, 2단계 인증 체크"),
...
def keyword_page(ids=None):
    """네 가지 방식을 화면 그림과 함께 하나씩. 접수 페이지에서 카드로 들어온다.

       ★ 여기서도 버튼 좌표 같은 화면 세부는 최소로 둔다. 다만 이 페이지는 '어떻게 하는지'가
         목적이라 버튼 이름은 쓴다 — 화면이 바뀌면 캡처와 함께 이 페이지를 고쳐야 한다."""
    return [
        erp_top(),
        ...
        h2("📍 지역형 — 원하는 동네에서 보이고 싶을 때"),
        para(t("지역과 제품 키워드를 곱해 조합을 만들고, 그중 인기글에 실제로 들어가는 것만 걸러 냅니다.")),
        img("03-지역형.png", "지역을 고르고 제품 키워드를 칩으로 쌓습니다"),
        cols([
            card("1️⃣", "지역 고르기", "복수 선택 가능"),
            card("2️⃣", "제품 키워드", "적고 「추가」"),
            card("3️⃣", "인기탭 찾기", "되는 것만 걸러짐"),
        ]),
        callout("📌", "gray_background", [t("이렇게 하시면 됩니다", bold=True)], [
            para(t("① 노출되고 싶은 지역을 모두 고릅니다. 안 고르시면 서울·경기·인천으로 찾습니다.")),
            para(t("② \"입주청소\"처럼 파시는 것을 적고 「추가」를 눌러 칩으로 쌓습니다.")),
            para(t("③ 「🎯 인기탭 찾기」를 누르면 지역 × 제품키워드 조합 중 인기글에 들어가는 것만 남습니다.")),
            para(t("· 「지역 키워드 생성」은 조합만 먼저 만들어 보는 버튼입니다.", color="gray")),
        ]),
        callout("⚠️", "yellow_background",
                [t("지역을 너무 많이 고르면 오래 걸립니다", bold=True)],
                [para(t("확인할 조합이 지역 수 × 키워드 수로 늘어납니다. 실제로 팔 지역만 고르세요.",
                        color="gray"))]),
        div(),

        h2("🏪 키워드형 — 네이버 플레이스가 있을 때"),
        para(t("플레이스 주소만 있으면 우리 업체가 실제로 잡히는 키워드를 찾아 드립니다.")),
        img("04-키워드형.png", "플레이스 주소를 붙여넣고 '정확 인기탭 분석'"),
        cols([
            card("1️⃣", "플레이스 주소", "지도에서 복사"),
            card("2️⃣", "정확 인기탭 분석", "수초~수십초"),
            card("3️⃣", "결과 고르기", "쓸 것만 체크"),
        ]),
        callout("📌", "gray_background", [t("이렇게 하시면 됩니다", bold=True)], [
            para(t("① 네이버 지도에서 우리 업체를 찾아 「공유」로 주소를 복사해 붙여넣습니다.")),
            para(t("② 「정확 인기탭 분석」을 누르면 실제 인기글 섹션을 확인합니다. 수초에서 수십 초 걸립니다.")),
            para(t("③ 나온 키워드 중 쓰실 것을 고릅니다.")),
            para(t("· 플레이스에 메뉴·소개가 부족하면 「📋 상세 정보 입력」에 소개글을 붙여넣어 주세요. "
                   "담당자가 키워드 선정에 씁니다.", color="gray")),
        ]),
        div(),

        h2("✍️ 직접 입력형 — 쓸 키워드가 이미 정해져 있을 때"),
        para(t("적어 주신 키워드 중 인기글에 실제로 들어가는 것만 골라 드립니다.")),
        img("05-직접입력형.png", "키워드를 하나씩 추가한 뒤 '인기탭 확인'"),
        cols([
            card("1️⃣", "키워드 추가", "최대 50개"),
            card("2️⃣", "인기탭 확인", "되는 것만 남음"),
            card("3️⃣", "고르기", "남은 것 중 선택"),
        ]),
        callout("📌", "gray_background", [t("이렇게 하시면 됩니다", bold=True)], [
            para(t("① 발행하고 싶은 키워드를 하나씩 적고 「추가」를 누릅니다. 최대 50개까지 담깁니다.")),
            para(t("② 다 모으셨으면 「인기탭 확인」을 누릅니다.")),
            para(t("③ 인기글 섹션에 실제로 들어가는 것만 남습니다. 그중에서 고르세요.")),
        ]),
        callout("💡", "yellow_background",
        ...
        callout("📌", "gray_background", [t("② 주소로 찾기", bold=True)], [
            para(t("홈페이지나 네이버 블로그 주소를 넣고 「⬇ 주소로 가져오기」를 누르면 글에서 키워드를 뽑아 냅니다.")),
