...
### 복원 팝업 경로는 **일부러 재현해서** 확인할 것

팝업은 복원거리가 있을 때만 뜨므로, 운 좋게 안 뜨면 그 코드는 한 번도 안 돌아본 채 배포된다.
이 경로가 고장나 있으면 **그 계정의 저장이 영구히 막히므로**, 아래처럼 만들어서 확인한다:

1. 글쓰기 화면에서 손으로 아무 글자나 몇 자 입력 (자동저장이 돌 때까지 ~30초 대기)
2. 탭을 닫거나 다른 페이지로 이동 (이탈 확인창이 뜨면 나가기)
3. `python save_blog.py --job <id>` (dry-run) 실행
4. 로그에 **`복원 팝업 감지 → [취소](새로 작성) 클릭`** 이 찍히는지 확인
5. 이어서 `비우기 전: …` 과 `[DRY] 저장 버튼 발견` 까지 정상 진행되는지 확인

- [ ] 위 5단계로 복원 팝업 처리 경로를 **라이브 확인**했다
- [ ] 팝업이 뜬 실행에서도 제목/본문이 이전 글과 섞이지 않았다
