...
- [ ] 로그에 **`저장요청 200`** — 클릭 이후 `RabbitTempPostWrite` 가 OK 로 응답
...
- [ ] 제목이 본문에 섞이거나 본문이 제목칸에 들어가지 않음 (제목/본문 셀렉터 분리 확인)
- [ ] 이전 저장분이 겹쳐 쓰이지 않음

> 🔴 **저장 카운터(`저장 N` → N+1)를 합격 근거로 쓰지 말 것.** 네이버가 타이핑 중에도
> 자동저장(`RabbitAutoSaveWrite`)을 돌려 카운터를 스스로 올린다(실측: 아무 조작 없이 0→1).
> 저장 버튼을 누르지 않아도 N+1 이 되므로 **거짓 성공**이 된다. 초기 기준이 이거였고,
> 실측으로 폐기했다. 카운터는 사람이 임시저장함에서 글을 찾는 참고값으로만 쓴다.

첫 스모크는 **헤드풀 + 사람 입회 + 테스트/자사 블로그**로. 통과 후에만 헤드리스 전환.

## 자주 나오는 실패

**`에디터를 비우지 못했습니다` + `Locator.click: Timeout 30000ms`**
네이버 복원 팝업("작성 중인 글이 있습니다 … 이어서 작성하시겠습니까?")이 떠서 딤이
화면을 덮은 상태다. 저장이 한 번이라도 중단되면(크래시·세션만료·자동저장) 복원거리가
생겨 매번 뜬다. `_dismiss_restore_popup` 이 **[취소]** 로 닫는다(2026-08-11 추가).

> 🔴 **[확인]을 누르면 안 된다.** 이전 글이 복원되고 우리 원고가 그 위에 덧붙는다 —
> `_clear_editor` 로 막으려던 바로 그 오염이 팝업 한 번으로 되살아난다.
> 그래서 버튼을 셀렉터만으로 고르지 않고 **텍스트('취소' 포함·'확인' 제외)로 검증**하고 누른다.
> '확인' 버튼 셀렉터는 아예 상수로 두지 않는다(복붙 사고 차단). T13 이 이 규칙을 강제한다.

증상이 헷갈리는 이유: 팝업 뒤에서도 **DOM 조회는 되기 때문에** `비우기 전: component=2 ·
내용=(없음)` 처럼 정상으로 보이는 로그가 찍힌다. 클릭만 막힌다.
