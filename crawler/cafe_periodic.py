...
#   (00:10~08:10 Full → 체인 카페측정 → 09:05 당일건 → 09:20 플레이스까지).
...
# ⚠️ BUSY_START 는 Full 크롤 시작 시각과 반드시 같거나 빨라야 한다.
#   2026-08-21 Full 을 01:00 → 00:10 으로 앞당겼는데 여기가 00:50 이라, 00:10 슬롯의
#   카페 전체 재측정(약 400글 · 35~40분)이 Full 과 정면으로 겹칠 뻔했다(독립검증에서 발견).
#   crawl_bydate.py 의 FULL_START 와 짝이다 — 한쪽만 고치면 다른 쪽이 조용히 뚫린다.
BUSY_START, BUSY_END = datetime.time(0, 0), datetime.time(9, 45)
