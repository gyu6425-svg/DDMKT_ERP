...
CASES = [
    ...
    # 2026-07-09 규칙 재정의(하이브리드 카운트) — 사용자 확정: 통합탭 = 파워링크 광고·이미지/동영상·플레이스만
    #   빼고, 위에서부터 화면 칸을 센다. 웹사이트(문서)도 순위 칸으로 포함(백석동 실측: 당근·구글 뒤 블로그=3위).
    #   · 웹사이트 블록 = 한 칸(같은 사이트 하위 링크가 여러 개여도 화면 한 칸).
    #   · 블로그/카페 인기글 블록 = 카드(r)마다 한 칸(한 블록 안 서로 다른 블로그는 각자 순위).
    #   ↳ 아래 기대값은 이 규칙으로 재기준화(이전 '웹사이트 제외'·'섹션내 순위' 값 폐기).
    ...
    ("인천연희동 통합탭", c._rank_in_popular, "통합탭_인천_연희동_누수탐지_2026_06_19.html", "rlawhddls125", 5, "ok"),
    ...
    ("유리교체 통합탭(블로그 1)", c._rank_in_popular, "통합탭_유리교체_2026_06_23.html", "ist3ist3", 9, "ok"),
    ("유리교체 통합탭(블로그 2)", c._rank_in_popular, "통합탭_유리교체_2026_06_23.html", "kimdo3040", 13, "ok"),
...
#   2026-07-09 재기준화: 웹사이트 포함 화면순. #1=224066671070 은 web 섹션 첫 칸=1위(이전 규칙에선 권외였음).
#   #2=224258926265 는 그 뒤 =2위. 224291228962 는 미노출(권외).
PERPOST_DUMP = "통합탭_likesign_글단위_2026_06_24.html"
PERPOST_CASES = [
    ("통합탭 blogId(아무 글이나)", "likesign", "", 1, "ok"),
    ("통합탭 글단위 #1글(web섹션)", "likesign", "224066671070", 1, "ok"),
    ("통합탭 글단위 #2글", "likesign", "224258926265", 2, "ok"),
    ("통합탭 글단위 추적글(권외)", "likesign", "224291228962", c.OUT_OF_RANK, "out"),
]

# 칠곡 업소용가구(pjyysh) 실측 — 2026-07-09 재기준화(웹사이트 포함 화면순): 카드 대표글 5/15글=5위.
#   6/11글(224312956224)은 afterArticles(관련글)에만 → 권외(발행일 다른 글 순위 전염 방지, 규칙 무관).
...
# 김포 경호업체(themansystem-) 실측 — 2026-07-09 재기준화: web(sks303040 문서) 포함 → 그 뒤 인기글 2위.
PERPOST_DUMP3 = "통합탭_김포경호업체_2026_06_24.html"
PERPOST3_CASES = [
    ("통합탭 더맨시스템", "themansystem-", "224299201732", 2, "ok"),
    # 멀티카드 회귀방지 — 인기글 블록 안 서로 다른 블로그(r=2·r=5)는 각자 순위. web 1칸 포함해 3·6위.
    ("통합탭 ugB 멀티카드 2위", "jhbillfallma", "224316244666", 3, "ok"),
    ("통합탭 ugB 멀티카드 5위", "gkstjeo97", "224317276845", 6, "ok"),
]

# 안산 푸르지오9차인테리어(design_do_) 실측 — 2026-07-09 재기준화: 위 웹사이트/문서 섹션 포함 화면순 → 6위.
PERPOST_DUMP4 = "통합탭_안산푸르지오9차_2026_06_24.html"
PERPOST4_CASES = [
    ("통합탭 안산 design_do_", "design_do_", "224266735547", 6, "ok"),
]

# 경기광주 인테리어필름(vision1803) 실측 — 2026-07-09 재기준화(웹사이트 포함 화면순): 위 당근/Moons(웹사이트)
#   포함해 6/22글=6위, 6/11글=8위, 5/11글=권외. blogId(아무 글)=첫 글 6위.
PERPOST_DUMP5 = "통합탭_경기광주인테리어필름_2026_06_25.html"
PERPOST5_CASES = [
    ("통합탭 경기광주 6/22글", "vision1803", "224323414074", 6, "ok"),
    ("통합탭 경기광주 6/11글", "vision1803", "224313044691", 8, "ok"),
    ("통합탭 경기광주 5/11글(권외)", "vision1803", "224281526330", c.OUT_OF_RANK, "out"),
    ("통합탭 경기광주 blogId(첫글)", "vision1803", "", 6, "ok"),
]
