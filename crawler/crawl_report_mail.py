...
def main():
    ...
    while datetime.datetime.now().time() < STOP_AT:
        ...
        if sent_start and not sent_mid and len(runs_of(st, "전체크롤")) > base_blog:
            ...
            send("[DDMKT] 크롤 중간 보고 — 블로그 완료",
                 ...
                 f"  참고: 2026-08-21 에 크롤을 약 1시간 앞당겼습니다 — 카페는 08:06 근처 종료가 정상입니다.\n"
                 f"  08:30 을 넘기면 대상 글이 늘어 여유가 줄었다는 뜻이니 알려주세요.\n")
            sent_mid = True
    ...
    if not sent_start:
        send("⛔ [DDMKT] 새벽 크롤이 시작되지 않았습니다",
             "00:10 크롤이 시작된 흔적이 없습니다(감시 종료 시각까지 running 이 한 번도 켜지지 않음).\n"
