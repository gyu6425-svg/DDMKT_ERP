...
def process_related(req, payload):
    ...
    PROBE_MAX_LIVE = 600                 # 약 25분(2.5초 간격). 캐시 히트는 여기 안 든다.
    #   400 → 600. 지역을 넓힌 만큼 예산도 올린다. 화면은 시간초과돼도 자동으로 이어붙으므로(172f342)
    #   길어지는 것 자체는 문제가 아니다. 남은 것은 note 에 명시하고 다음 조회가 캐시를 건너뛰고 이어 본다.
