...
export default function AgencyOrgPanel() {
    ...
    return (
        <section className="grid gap-5">
            ...
            {/* 요약·정산은 통합 대시보드로 옮겼다(사장님 확정 2026-08-20).
                같은 숫자를 두 화면에서 각각 계산하면 언젠가 갈라진다 — 계산식은 한 곳에만 둔다.
                여기는 '처리하는 곳'(승인·금액 통보·발행)이다. */}
            <p className="m-0 -mt-1 text-[13px] text-[#64748b]">
                하위 업체 가입 승인과 충전 신청을 처리합니다.
                {' '}현황·정산은 <button className="font-semibold text-[#1e40af] underline" onClick={() => { window.history.pushState(null, '', '/portal'); window.dispatchEvent(new Event('app:navigate')); }} type="button">통합 대시보드</button>에서 보실 수 있습니다.
            </p>
