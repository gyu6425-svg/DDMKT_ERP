...
function ContractAddModal({
    ...
    const isOutOnly = isBuy || isLand || isExp; // 외주비 전용 항목(매출 0)
    const cat =
        PRODUCT_CATEGORIES.find((c) => c.key === catKey) ??
        (isFee ? FEE_CAT : isBuy ? BUY_CAT : isLand ? LAND_CAT : isExp ? EXP_CAT : isEtc ? ETC_CAT : PRODUCT_CATEGORIES[0]);
    ...
    const chipCats = allCategorySubs
        ...
        : boostPrefix
          ? PRODUCT_CATEGORIES
          : [...PRODUCT_CATEGORIES, FEE_CAT, BUY_CAT, LAND_CAT, EXP_CAT];
    ...
    const pickCat = (key: string) => {
        ...
        if (key === BUY_KEY || key === LAND_KEY || key === EXP_KEY) {
            setSubtype(''); // 아이디 구매·랜딩페이지·체험단 = 이름 직접 입력
            return;
        }
                        ...
                        <>
                            ...
                            <label className="block text-xs font-semibold text-[#475569]">
                                ...
                                <input
                                    ...
                                    placeholder={
                                        isExp ? '예: 인스타 체험단 A' : isLand ? '예: 상세 랜딩페이지 제작' : '예: 네이버 아이디'
                                    }
                                    type="text"
                                    value={subtype}
                                />
...
export function ClientDetail({
    ...
    return (
        <section className="grid gap-4">
            ...
            {/* 누적 금액 — 순매출 = 공급가(VAT 제외) − 받은 외주비. 각 박스 누르면 상품별 내역 */}
            <div className="flex items-stretch gap-2">
                ...
                <button
                    ...
                    type="button"
                >
                    <div className="text-[11px] font-semibold text-[#94a3b8]">받은 외주비</div>
                    <div className="mt-0.5 text-lg font-bold text-[#dc2626] sm:text-2xl">
                        {fmtWon(receivedTotal)}원
                    </div>
                    <div className="mt-0.5 text-[10px] text-[#94a3b8]">
                        실제 사용 {fmtWon(usedTotal)} · 차액 {fmtWon(outMargin)}
                    </div>
                </button>
            </div>
                                                ...
                                                <span className="min-w-0">
                                                    ...
                                                    <span className="block text-[11px] text-[#94a3b8]">
                                                        ...
                                                        ct.outsource_company
                                                            ...
                                                            : ''}
                                                        {breakdown === 'net'
                                                            ? ` · 공급가 ${fmtWon(ct.amount || 0)} − 받은외주 ${fmtWon(ct.outsource || 0)}`
                                                            : ''}
                                                    </span>
                                                </span>
                                ...
                                breakdown === 'outsource'
                                    ...
                                    : breakdown === 'net'
                                      ? ([
                                            ...
                                            ['받은 외주비', `${fmtWon(detailC.outsource || 0)}원`, '#dc2626'],
                                            [
                                                '순매출 (공급가 − 받은 외주비)',
