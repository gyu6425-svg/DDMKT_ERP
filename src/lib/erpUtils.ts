...
// '신규 등록 건' 판정 컷오프 — 이 시점(2026-07-07) 이후 생성된 계약/블로그만 신규로 표시.
//   브랜드블로그 계약 관리 연동 일괄 반영 후 리셋 → 기존 건은 전부 승인(계약 중), 지금부터 등록분만 신규.
export const NEW_CONTRACT_CUTOFF_MS = 1783405466511;
...
// 계약별 실매출 — 우선순위: ① 저장된 실매출(부가세포함 합계, 이카운트 등 실제 세금계산서 금액) →
//   ② 현금(no_vat)이면 공급가 그대로 → ③ 공급가 × 1.1.
//   saleTotal 이 있으면 부가세를 재계산하지 않고 그 값을 그대로 써서 외부 장부(이카운트)와 원 단위까지 일치시킨다.
export const saleVat = (
    amount: number | null | undefined,
    noVat?: boolean | null,
    saleTotal?: number | null,
): number =>
    saleTotal != null && Number(saleTotal) > 0
        ? Number(saleTotal)
        : noVat
          ? Number(amount) || 0
          : withVat(amount);
