...
export async function customerLogin(clientId: string): Promise<{ ok: boolean; error?: string; reached?: boolean }> {
    try {
        ...
        // reached=true: 브릿지에 도달함(응답 받음). ok=false면 크롬 실행 실패(프로필 잠금·포트충돌 등) — d.error 로 사유.
